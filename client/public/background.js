...
window.addEventListener('load', () => {
  ...
  const rMax = 10;
  let canv; let
    ctx; // canvas and context : global variables (I know :( )
  let maxx; let
    maxy; // canvas sizes (in pixels)
  let particles;
  let click;
  let initDir;
  let noiseInitDir;
...
*/
    ...
    const arP0 = []; // 'preceeding value' for each harmonic
    const arP1 = []; // 'succeding value'
    const amplitudes = []; // amplitudes oh harmonics
    const increments = []; // n / period, wich will be added to phases for every point
    const phases = [];
    let globAmplitude = 0;
...
*/
  ...
  function startOver() {
    ...
    hue = intAlea(360);
    if (org.x == 0 && org.y == 0) { org = { x: maxx / 2, y: maxy / 2 }; }
